...
{
    // --- Твой хак, который ты уже вставил ---
    extern EquinoxAPI* g_sys;
    g_sys->print("WAD Loader: Bypassing fopen()...");
    
    uint64_t size = 0;
    lumpinfo = (lumpinfo_t*)g_sys->get_file("doom1.wad", &size);

    if (!lumpinfo)
    {
        g_sys->print("FATAL: Kernel could not provide WAD data!");
        while(1);
    }
    
    numlumps = size; 
    g_sys->print("WAD data located in memory!");

    // 2. ВОЗВРАЩАЕМ NULL, ЧТОБЫ КОМПИЛЯТОР БЫЛ ДОВОЛЕН
    return NULL;
}
